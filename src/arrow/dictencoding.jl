...
function DictEncoding(::Type{<:Array}, x::CategoricalArray)
    b = Vector{UInt8}(undef, totalbytes(x))
    DictEncoding(b, 1, x)
end
...
# Ported to CategoricalArrays 1.x: the pool's ordered level vector was `pool.index`
# up to 0.8 and is `pool.levels` from 0.9 on. Note this deliberately reads the raw
# field rather than calling `levels(x)`, which now returns `CategoricalValue`s instead
